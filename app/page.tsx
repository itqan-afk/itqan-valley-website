...
const serviceCatalog = [
  {
    ...
    items: [
      ["ملف الانطلاق", "سيرة احترافية متوافقة مع أنظمة الفرز ATS."],
      ["واجهة الخبرة", "إعادة بناء LinkedIn ليعرض قيمتك المهنية بوضوح."],
      ["رسالة الوصول", "خطاب يربط خبرتك مباشرة بالفرصة المستهدفة."],
      ["سجل القيادة", "سيرة تنفيذية تبرز الأثر والقرارات والإنجازات."],
      ["مرآة الملف", "تشخيص دقيق للسيرة مع توصيات قابلة للتطبيق."],
      ["حضور 360", "سيرة باللغتين وLinkedIn وخطاب تقديم متكامل."],
    ],
  },
  {
    audience: "للأعمال",
    ...
    items: [
      ["حلقة الأداء", "ربط المؤشرات والنتائج والإجراءات في دورة واحدة."],
      ["عدسة الإدارة", "لوحة قياس مصممة وفق أولويات المنشأة."],
      ["سجل القرار", "متابعة القرار من الاجتماع حتى التنفيذ والإغلاق."],
      ["قراءة القيادة", "تحويل البيانات إلى موجز واضح يدعم القرار."],
      ["نسيج التشغيل", "توثيق الأدوار والاعتمادات ومسارات العمل."],
    ],
  },
];
...
const workflowBridge = ["المدخل", "التحليل", "الإجراء", "القرار", "الأثر"];
const navItems = ["الرئيسية", "حلولنا", "الخدمات", "أعمالنا", "منهجيتنا", "تواصل معنا"];
    ...
    <div className="slides-window" onTouchStart={(event) => { touchStart.current = event.touches[0].clientX; }} onTouchEnd={(event) => {
      ...
      <div className="slides-track" style={{ transform: `translate3d(-${active * 100}%, 0, 0)` }}>
        ...
        <section className="slide workflow-slide" aria-hidden={active !== 4}>
          ...
          <div className="workflow-bridge" aria-label="مسار تحويل الاحتياج إلى أثر">
            <strong>مسار متصل</strong>
            <div>{workflowBridge.map((item, index) => <span key={item}><b>{String(index + 1).padStart(2, "0")}</b>{item}</span>)}</div>
          </div>
